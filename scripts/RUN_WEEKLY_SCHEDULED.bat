@echo off
REM ============================================================
REM  HRP-Ops WEEKLY SCHEDULED (non-interactive)
...
echo [%date% %time%] Weekly scheduled run starting >> outputs\scheduled_run.log
...
if errorlevel 1 (
    echo [%date% %time%] LOGIN REQUIRED: Session expired. Run scripts\RUN_WEEKLY.bat manually. >> outputs\scheduled_run.log
    echo.
    echo ========================================
    echo  LOGIN REQUIRED
    echo  Session expired. Run manually:
    echo    scripts\RUN_WEEKLY.bat
    echo ========================================
    .venv\Scripts\python.exe scripts\07_write_run_status.py 2 auth_expired
    exit /b 2
)

REM Step 3: Export (weekly = full 8 pages/horse)
...
if %PIPELINE_OK%==1 (
    ...
    echo [%date% %time%] SUCCESS: Weekly run complete >> outputs\scheduled_run.log
) else (
    .venv\Scripts\python.exe scripts\07_write_run_status.py 2 partial_export
    echo [%date% %time%] PARTIAL: Weekly run completed with partial export >> outputs\scheduled_run.log
)
